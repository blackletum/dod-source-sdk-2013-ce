...
//----------------------------------------------------------------------------------
bool CHeadlessHatmanAttack::IsPotentiallyChaseable( CHeadlessHatman *me, CDODPlayer *victim )
{
	if ( !victim )
	{
		return false;
...
{
	CDODPlayer *victim = ToDODPlayer( DODGameRules()->GetIT() );
...
void CHeadlessHatmanAttack::SelectVictim( CHeadlessHatman *me )
{
	...
	m_closestVisible = ToDODPlayer( DODGameRules()->GetIT() );

	if ( DODGameRules()->GetIT() == NULL )
	{
		// pick a new victim to chase
		CDODPlayer *newVictim = NULL;
	...
	{
		m_attackTarget = ToDODPlayer( DODGameRules()->GetIT() );
	}
}
...
void CHeadlessHatmanAttack::AttackTarget( CHeadlessHatman *me, CBaseCombatCharacter *pTarget, float flAttackRange )
{
	...
	toVictim.NormalizeInPlace();
		...
		if ( DotProduct( forward, toVictim ) > hitAngle )
		{
			...
			{
				// CHOP!
				CTakeDamageInfo info( me, me, m_attackTarget->GetMaxHealth() * 0.8f, DMG_CLUB, NULL );
...
ActionResult< CHeadlessHatman >	CHeadlessHatmanAttack::Update( CHeadlessHatman *me, float interval )
{
	...
	else
	{
		...
		CDODPlayer *chaseVictim = ToDODPlayer( DODGameRules()->GetIT() );
