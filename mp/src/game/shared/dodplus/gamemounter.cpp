...
void MountPathLocal(KeyValues* pGame)
{
	...
	if (ccFolder > 0)
	{
		ConColorMsg(Color(90, 240, 90, 255), "[GAMEMOUNTER] Mounting %s (local)\n", szGameName);
...
void MountPathDedicated(KeyValues* pGame)
{
	...
	ConColorMsg(Color(90, 240, 90, 255), "[GAMEMOUNTER] Mounting %s (dedicated)\n", szGameName);
...
void MountSourceMod(KeyValues* pGame)
{
	...
	if (ccFolder > 0)
	{
		ConColorMsg(Color(90, 240, 90, 255), "[GAMEMOUNTER] Mounting %s (sourcemod)\n", szGameName);
...
{
#ifdef PMMOD
	PMMOD_GameMounter_Log("Initializing GameMounter\n");
#endif
