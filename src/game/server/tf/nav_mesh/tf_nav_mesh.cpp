...
ConVar doc_bot_min_setup_gate_defend_range( "doc_bot_min_setup_gate_defend_range", "750", FCVAR_CHEAT, "How close from the setup gate(s) defending bots can take up positions. Areas closer than this will be in cover to ambush." );
ConVar doc_bot_max_setup_gate_defend_range( "doc_bot_max_setup_gate_defend_range", "2000", FCVAR_CHEAT, "How far from the setup gate(s) defending bots can take up positions" );
...
void CTFNavMesh::ComputeBlockedAreas()
{
	...
	while ( ( pDoor = (CBaseToggle *)gEntList.FindEntityByClassname( pDoor, "func_door*" ) ) != NULL )
	{
		...
		if ( iBlockedTeam > TEAM_UNASSIGNED )
			iNavTeam = ( iBlockedTeam == TEAM_ALLIES ) ? TEAM_AXIS : TEAM_ALLIES;
...
void CTFNavMesh::ComputeIncursionDistances()
{
	...
	{
		CTFNavArea *area = static_cast<CTFNavArea *>( TheNavAreas[i] );
		area->SetIncursionDistance( TEAM_ALLIES, -1.0f );
	...
	{
		CTFNavArea *area = static_cast<CTFNavArea *>( TheNavAreas[i] );
		flMaxDistance = Max( flMaxDistance, area->GetIncursionDistance( TEAM_AXIS ) );
	}

	for ( int i=0; i<TheNavAreas.Count(); ++i )
	{
		CTFNavArea *area = static_cast<CTFNavArea *>( TheNavAreas[i] );
		float flIncursionDist = area->GetIncursionDistance( TEAM_AXIS );
		if ( flIncursionDist >= 0.0f )
			area->SetIncursionDistance( TEAM_ALLIES, ( flMaxDistance - flIncursionDist ) );
	}*/
}
	...
	if ( !engine->IsDedicatedServer() )
	{
		...
		if ( host && host->IsConnected() )
		{
			for ( int i=0; i<TheNavAreas.Count(); ++i )
			{
				...
				{
					CUtlString string;
					if ( area->IsBlocked( TEAM_ALLIES ) && area->IsBlocked( TEAM_AXIS ) )
						string = "Blocked for All";
					else if ( area->IsBlocked( TEAM_ALLIES ) )
						string = "Blocked for Red";
					else if ( area->IsBlocked( TEAM_AXIS ) )
