	...
	bool operator()( CNavArea *a )
	{
		...
		for ( int dir=0; dir<NUM_DIRECTIONS; ++dir )
		{
			for ( int i=0; i<area->GetAdjacentAreas( (NavDirType)dir )->Count(); ++i )
			{
				...
				if ( area->GetIncursionDistance( TEAM_AXIS ) <= other->GetIncursionDistance( TEAM_AXIS ) ||
					 area->GetIncursionDistance( TEAM_AXIS ) > m_pArea->GetIncursionDistance( TEAM_AXIS ) + 100.0f )
					continue;

				m_redAreas->AddToTail( other );

				if ( area->GetIncursionDistance( TEAM_ALLIES ) <= other->GetIncursionDistance( TEAM_ALLIES ) ||
					 area->GetIncursionDistance( TEAM_ALLIES ) > m_pArea->GetIncursionDistance( TEAM_ALLIES ) + 100.0f )
					continue;
...
bool CTFNavArea::IsBlocked( int teamID, bool ignoreNavBlockers ) const
{
	if ( !( m_nAttributes & UNBLOCKABLE ) )
	{
		...
		{
			if ( teamID != TEAM_ALLIES )
			{
				if ( teamID == TEAM_AXIS && ( m_nAttributes & RED_ONE_WAY_DOOR ) )
					return true;
...
void CTFNavArea::Draw() const
{
	...
	if ( tf_nav_show_incursion_distances.GetBool() )
	{
		NDebugOverlay::Text( GetCenter(),
							 UTIL_VarArgs( "R:%3.1f   B:%3.1f", m_aIncursionDistances[TEAM_ALLIES], m_aIncursionDistances[TEAM_AXIS] ),
	...
	if ( tf_nav_show_turf_ownership.GetBool() )
	{
		...
		bool bRedOwnsMe = IsAwayFromInvasionAreas( TEAM_ALLIES, flRadius );
		bool bBluOwnsMe = IsAwayFromInvasionAreas( TEAM_AXIS, flRadius );
	...
	if ( tf_show_blocked_areas.GetBool() )
	{
		...
		if ( IsBlocked( TEAM_ALLIES ) && IsBlocked( TEAM_AXIS ) )
			DrawFilled( 100, 0, 100, 255 );
		else if ( IsBlocked( TEAM_ALLIES ) )
			DrawFilled( 100, 0, 0, 255 );
		else if ( IsBlocked( TEAM_AXIS ) )
...
void CTFNavArea::ComputeInvasionAreaVectors()
{
	...
	CollectInvasionAreas functor( this, &m_InvasionAreas[TEAM_ALLIES], &m_InvasionAreas[TEAM_AXIS], searchMarker );
